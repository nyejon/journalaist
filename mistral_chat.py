...
if st.session_state.page == 'chat':
    ...
    if "n_pictures" not in st.session_state:
    ...
    with col2:
        upload_image = st.button("Upload Image", on_click=show_upload, args=[True])
    ...
    if upload_image:
        if st.session_state["uploader_visible"]:
            ...
            st.session_state.current_file = st.file_uploader("Choose images...", type=["jpg", "png"], accept_multiple_files=False)
            #st.session_state.messages.append(SystemMessage(content=f"Uploaded {len(uploaded_files)} images"))
            print("Here")

    print("Current file",  st.session_state.current_file)
    if st.session_state.current_file:
        if not os.path.exists("./stories"):
            os.makedirs("./stories")

        cols = st.columns(len(st.session_state.current_file))
        for col, uploaded_file in zip(cols, st.session_state.current_file):
